...
import gc, { IGarminTokens } from "npm:garmin-connect-nexxt@1.6.24";
...
export async function sync(ctx: Context): Promise<SyncResult> {
  ...
  const activitiesCount = ctx.trigger === "sync_all" ? 5 : 3;
  ...
  for (const activity of activities) {
    ...
    if (
      ctx.trigger === "daily_sync" &&
      !isToday(activity.startTimeGMT) &&
      !isToday(endTimeGMT)
    ) {
  ...
  if (
    (ctx.trigger === "sync_all" && dailySyncContinuation) ||
    (ctx.trigger === "sync_all" && activities.length === activitiesCount)
  ) {
    continuation = {
      state: {
        ...ctx.state,
